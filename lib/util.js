...
Util.prototype.handleCompanyDisclosure = function(companyhtml, callback) {
  ...
  callback(companyDisclosure)
}

//---------------------------------------------------

Util.prototype.handleReportDetail = function(report, callback) {
  var self = this;
  ...
  if (report.hasOwnProperty('type') && report.type === 'url') {
    ...
    homeClient.get(url, function(err, req, res, body) {
      ...
      var companyOtherInfosObj = $("table")[2];
      var companyOtherInfos = InfoLists2($, $(companyOtherInfosObj));
      callback(null, {
        basicInfo: basicInfo,
        sitesInfo: sitesInfo,
        companyOtherInfos: companyOtherInfos
      });
    });
  }
  if(self.reportflag === false) {
    callback(null);
  }
}