 ...
 */
var getPlatformParameter = function(callback) {
 ...
 */
var getSingleCreditHTML = function(options, callback) {
 ...
 */
var getDisclosureHTML = function(options, callback) {
  log(options)
...
getPlatformParameter(function(err, obj) {
  log(err, obj);
  obj.keyword = "310109000309513";
  var Cookie = obj.Cookie;
  getSingleCreditHTML(obj, function(err, body) {
    log(body)
    var options = {
      uuid: body.uuid,
      tab: '01',
    ...
    log(options)
    getDisclosureHTML(options, function(err, body) {
      log(err,body)
    })
  })
})